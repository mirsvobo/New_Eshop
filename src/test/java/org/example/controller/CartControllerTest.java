...
class CartControllerTest {
    ...
    @Test
    @WithMockUser
    void addToCart_WithBackendDefaultVariants_StoresTheirTextSnapshots() throws Exception {
        when(productRepository.findById(1L)).thenReturn(Optional.of(configurableProduct()));
        when(productImageLayerService.validateAndResolveSelection(
                1L,
                LayerType.LAZURE,
                "Afromorsia"
        )).thenReturn("Afromorsia");
        when(productImageLayerService.validateAndResolveSelection(
                1L,
        ...
        mockMvc.perform(post("/kosik/pridat")
                        ...
                        .param("productId", "1")
                        .param("quantity", "1")
                        .param("selectedLazure", "Afromorsia")
                        .param("selectedRoofColor", "Antracit"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/kosik"));
        ArgumentCaptor<CartItemDto> captor = ArgumentCaptor.forClass(CartItemDto.class);
        verify(cart).addItem(captor.capture());
        assertEquals("Afromorsia", captor.getValue().getSelectedLazure());
        assertEquals("Antracit", captor.getValue().getSelectedRoofColor());
    }
    @Test
    @WithMockUser
